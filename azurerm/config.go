...
func getArmClient(c *authentication.Config) (*ArmClient, error) {
	...
	client.registerHDInsightClients(endpoint, c.SubscriptionID, auth)
...
func (c *ArmClient) registerHDInsightClients(endpoint, subscriptionId string, auth autorest.Authorizer) {
	hic := hdinsight.NewClustersClientWithBaseURI(endpoint, subscriptionId)
	c.configureClient(&hic.Client, auth)
	c.clustersClient = hic
}
