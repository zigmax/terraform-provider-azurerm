...
func TestAccAzureRMApplicationGateway_basic_changeSubnet(t *testing.T) {
	resourceName := "azurerm_application_gateway.test"
	ri := acctest.RandInt()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testCheckAzureRMApplicationGatewaySslCertificateAssigned(resourceName, "ssl-1"),
					testCheckAzureRMApplicationGatewayAuthenticationCertificateAssigned(resourceName, "auth-1"),
					resource.TestCheckResourceAttr(resourceName, "gateway_ip_configuration.0.subnet_id", fmt.Sprintf("subnet-%d", ri)),
				),
				Destroy: false,
			},
			{
				Config: testAccAzureRMApplicationGateway_basicChangeSubnet(ri, testLocation()),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMApplicationGatewayExists(resourceName),
					testCheckAzureRMApplicationGatewaySslCertificateAssigned(resourceName, "ssl-1"),
					testCheckAzureRMApplicationGatewayAuthenticationCertificateAssigned(resourceName, "auth-2"),
					resource.TestCheckResourceAttr(resourceName, "gateway_ip_configuration.0.subnet_id", "second-subnet"),
				),
			},
		},
	})
...
func testAccAzureRMApplicationGateway_basicChangeSubnet(rInt int, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestrg-%d"
  location = "%s"
